...
ratioArray=(0.34 0.67 1.34 1.67 2) # hard coded for now
...
do
    ...
    do
        pry_cons_2=`echo "scale=0; ${ratioArray[$k]}*$pry_cons_1" | bc`;
        pry_cons_2=${pry_cons_2%.*} # floor trick
        
        divide=$freq_surv*$pry_cons_2; by=3; pry_surv_2=`echo "scale=0; ($divide+$by-1)/$by" | bc`; # argv[17]
        pry_repr_2=$pry_surv_2; # argv[18] 

        echo "predators specific: $prd_spcf_1 ; prey2 to prey1 max consume ratio : ${ratioArray[$k]}"

        # name the simulation with only the variables of interest and their value
        sim_name="localSA-predSpcf$prd_spcf_1-predOpnt$prd_oprt_1-pryConvRateRatio1-pryCtchProbRatio1-pryOfspRatio1-pryMaxConsRatio${ratioArray[$k]}" # argv[1]
        ...
        cp localSA-chapter2ibm.o $sim_name/localSA-chapter2ibm.o
