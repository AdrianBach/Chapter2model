...
ratioArray=($(seq 2 1 10)) # hard coded for now
...
do
    ...
    do
        pry_offs_2=$((${ratioArray[$k]}*$pry_offs_1)); echo "pry_offs_2 is $pry_offs_2"

        echo "predators specific: $prd_spcf_1 ; prey2 to prey1 average offspring number ratio : ${ratioArray[$k]}"

        # name the simulation with only the variables of interest and their value
        sim_name="localSA-predSpcf$prd_spcf_1-predOpnt$prd_oprt_1-pryConvRateRatio1-pryCtchProbRatio1-pryOfspRatio${ratioArray[$k]}-pryMaxConsRatio1" # argv[1]
