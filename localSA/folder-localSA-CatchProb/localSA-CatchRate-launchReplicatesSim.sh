...
ratioArray=(0.1 0.5 1.5 2 2.5 3) # hard coded for now
...
do
    ...
    do
        prd_ctch_pry2_1=`echo "scale=2; ${ratioArray[$k]}*$prd_ctch_pry1_1" | bc`; echo "prd_ctch_pry2_1 is $prd_ctch_pry2_1"

        echo "predators specific: $prd_spcf_1 ; prey2 to prey1 catch proba ratio : ${ratioArray[$k]}"

        # name the simulation with only the variables of interest and their value
        sim_name="localSA-predSpcf$prd_spcf_1-predOpnt$prd_oprt_1-pryConvRateRatio1-pryCtchProbRatio${ratioArray[$k]}-pryOfspRatio1-pryMaxConsRatio1" # argv[1]
